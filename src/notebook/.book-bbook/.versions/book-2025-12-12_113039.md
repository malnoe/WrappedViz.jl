...
```julia (editor=true, logging=false, output=true)
#calculer le temps d'écoute total pour chaque artiste
df_artist_time = combine(groupby(df, :artist), :ms_played => sum => :total_time)
#trier par temps d'écoute décroissant
sort!(df_artist_time, :total_time => x-> x/60000 => t, rev=true)
#sélectionner les 10 artistes avec le plus de temps d'écoute
top10_artists_time = df_artist_time[1:10, :]
```
