...
```julia (editor=true, logging=false, output=true)
tps_min = sum(df.duration_ms) / 60000  # en minutes
tps_h = tps_min / 60
tps_j = tps_h / 24

printstyled("Vous avez $(round(tps_min, digits = 0)) minutes d'écoute, soit $(round(tps_h, digits = 0)) heures, ou $(round(tps_j)) jours. Waouh !")
```
