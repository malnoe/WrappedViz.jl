...
function spiderplot(df_freq::DataFrame; labelcol::Symbol=:hour, valcol::Symbol=:freq,
    ...
    df = sort(df_freq, labelcol,rev=true)  # si tes heures sont 0..23, ça les ordonne
    labels = string.(df[!, labelcol])
    r = Float64.(df[!, valcol])

    n = length(r)
    θ = range(0, 2π; length=n+1)[1:end-1] |> collect
    ...
    # rayons
    for t in range(0, 2π; length=n+1)[1:end-1]
    ...
    # labels
    θlab = range(0, 2π; length=n+1)[1:end-1]
