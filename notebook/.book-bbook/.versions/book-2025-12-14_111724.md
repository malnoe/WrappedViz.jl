...
printstyled("Vous avez $(round(tps_min, digits = 2)) minutes d'écoute, soit $(round(tps_h, digits = 2)) heures, ou $(round(tps_j, digits = 0)), waouh !")
```
...
fig
```
## 3. Répartition sur les heures de la semaine
...
```julia (editor=true, logging=false, output=true)
printstyled("Vous avez écouté $(length(unique(df.track_name))) chansons uniques.")
