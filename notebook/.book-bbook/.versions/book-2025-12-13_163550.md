...
# Exemple : df_freq déjà préparé
# df_freq = DataFrame(hour=0:23, freq=[...])

max_radius = maximum(df_freq.freq)

# DataFrame pour les labels radiaux
label_df = DataFrame(
    hour = 0:23,
    radius = max_radius .+ 5,   # décale les labels à l'extérieur
    label = string.(0:23)
)

# Rose des vents + labels radiaux
@vlplot(
    width = 400,
    height = 400,
    layer = [
        # Arc principal
        df_freq |> @vlplot(
            mark = { :arc, innerRadius = 20, stroke = "white" },
            encoding = {
                theta = { field = :hour, type = :quantitative,
                          scale = {domain = [0,24], range = [π/2, 5π/2]} },
                radius = { field = :freq, type = :quantitative, title = "Fréquence d'écoute" },
        ...
        # Labels radiaux
        label_df |> @vlplot(
            mark = { :text, align="center", baseline="middle", angle= { field=:hour, type=:quantitative, scale={domain=[0,24], range=[0,360]} } },
            encoding = {
