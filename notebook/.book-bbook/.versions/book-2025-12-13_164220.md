...
# Exemple de données
df_freq = DataFrame(hour=0:23, freq=rand(24) .* 10)

max_radius = maximum(df_freq.freq)

# Labels radiaux pour les heures
label_df = DataFrame(
    hour = 0:23,
    radius = max_radius .+ 2,   # décale les labels à l'extérieur
    label = string.(0:23)
)

# Couche arcs
arcs = df_freq |> @vlplot(
    mark = { :arc, innerRadius = 0, stroke = "white" },
    encoding = {
        theta = { field = :hour, type = :quantitative,
                  scale = {domain = [0,24], range = [π/2, 5π/2]} },
        radius = { field = :freq, type = :quantitative },
        color = { field = :hour, type = :quantitative, scale={scheme="turbo"}, legend=nothing },
        tooltip = [
            {field=:hour, type=:quantitative, title="Heure"},
            {field=:freq, type=:quantitative, title="Écoutes"}
        ]
...
# Couche labels
labels = label_df |> @vlplot(
    mark = { :text, align="center", baseline="middle" },
    encoding = {
        theta = { field = :hour, type = :quantitative, scale={domain=[0,24], range=[π/2,5π/2]} },
        radius = { field = :radius, type = :quantitative },
        text = { field = :label, type = :nominal },
        # rotation pour que le texte suive le cercle
        angle = { field=:hour, type=:quantitative, scale={domain=[0,24], range=[-90,270]} }
    }
)

# Combiner les deux couches
(arcs + labels) |> @vlplot(width=400, height=400, config={view={stroke=nothing}})
```
